// controllers/ProjectTestiClientController.ts
import { Request, Response } from "express";
import {
  ProjectTestiClientModel,
  ProjectTestiClientData,
} from "../models/ProjectTestiClient";
...
export class ProjectTestiClientController {
  async getAllProjectTestiClients(req: Request, res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch project client testimonials" });
    }
  }
  ...
  async createProjectTestiClient(req: Request, res: Response) {
    try {
      ...
      const newProjectTestiClient = await ProjectTestiClientModel.create(
        projectTestiClientData
      );
  ...
  async updateProjectTestiClient(req: Request, res: Response) {
    try {
      ...
      if (!updatedProjectTestiClient) {
        return res
          .status(404)
          .json({ error: "Project client testimonial not found" });
      }
  ...
  async deleteProjectTestiClient(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const deletedProjectTestiClient = await ProjectTestiClientModel.delete(
        parseInt(id)
      );
